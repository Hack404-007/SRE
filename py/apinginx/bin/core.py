...
class Pygtail(object):
    ...
    def __init__(self, filename, offset_file=None, paranoid=False):
        ...
        if exists(self._offset_file):
            ...
            offset_fh.close()
            print "Saved offset:", self._offset
            if self._offset_file_inode != stat(self.filename).st_ino:
                ...
                print "st_size:", stat(self.filename).st_size
                # Saved offset larger than file size. It must be a bug somewhere...
                # Rotate the log as a workaround
                if self._offset > stat(self.filename).st_size:
                    print "Go back to file begin"
                    self._offset = 0
    ...
    def readline(self):
        """
        Return te next line in the file without buffering, updating the offset.
        """
    ...
    def _check_rotated_filename_candidates(self):
        ...
        """
        # pushlog
        candidate = "%s1.log" % self.filename[:-4]
        if (exists(candidate)):
            return candidate
